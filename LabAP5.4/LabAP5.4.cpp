...
double S0(const int K, const int N)
{
	double s = 0;
	for (int i = K; i <= N; i++)
		s += sin(1. * i) * cos(1. * i) / (1 + pow(sin(1.0 * i), 2));
	return s;
}
double S1(const int K, const int N, const int i)
{
	if (i > N)
		...
		return sin(1. * i) * cos(1. * i) / (1 + pow(sin(1.0 * i), 2)) + S1(K, N, i + 1);
}
double S2(const int K, const int N, const int i)
{
	if (i < K)
		...
		return sin(1. * i) * cos(1. * i) / (1 + pow(sin(1.0 * i), 2)) + S2(K, N, i - 1);
}
double S3(const int K, const int N, const int i, double t)
{
	t = t + sin(1. * i) * cos(1. * i) / (1 + pow(sin(1.0 * i), 2));
	if (i >= N)
		return t;
	else
		return S3(K, N, i + 1, t);
